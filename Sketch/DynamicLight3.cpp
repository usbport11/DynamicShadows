...
DynamicLight3* DynamicLight3::create(cocos2d::backend::ProgramState* shadowMap, cocos2d::backend::ProgramState* shadowRender) {
	DynamicLight3 *pRet = new(std::nothrow) DynamicLight3();
    if (pRet && pRet->init(shadowMap, shadowRender)) {
        pRet->autorelease();
        return pRet;
    }
    else {
...
bool DynamicLight3::init(cocos2d::backend::ProgramState* shadowMap, cocos2d::backend::ProgramState* shadowRender) {
	if (!Node::init()) {
        return false;
    }
	...
	shadowMapShaderState = shadowMap;
	shadowRenderShaderState = shadowRender;
