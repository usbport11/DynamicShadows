...
DynamicLight3::~DynamicLight3() {
	...
	CC_SAFE_RELEASE(lightCircle);
}

void DynamicLight3::initOcclusionMap() {
	CC_SAFE_RELEASE(occlusionMap);
	CC_SAFE_RELEASE(occlusionMapSprite);
...
void DynamicLight3::initShadowMap1D() {
	CC_SAFE_RELEASE(shadowMap1D);
	CC_SAFE_RELEASE(shadowMap1DSprite);
...
void DynamicLight3::initFinalShadowMap() {
	CC_SAFE_RELEASE(finalShadowMap);
	CC_SAFE_RELEASE(finalShadowMapSprite);
...
void DynamicLight3::updateUniforms() {
...
void DynamicLight3::createOcclusionMap() {
	if (!shadowCaster) {
...
void DynamicLight3::create1DShadowMap() {
	//for debug draw only
...
void DynamicLight3::createFinalShadowMap() {
...
cocos2d::backend::Program* DynamicLight3::loadShader(const std::string& vert, const std::string& frag) {
...
bool DynamicLight3::createLight() {
	if (!bakedMapIsValid) {
...
void DynamicLight3::setPosition(const cocos2d::Point& position) {
...
void DynamicLight3::setSoftShadows(bool shadows) {
	softShadows = shadows;
}

void DynamicLight3::setLightSize(int size) {
...
void DynamicLight3::setShadowCaster(cocos2d::Sprite* caster) {
    bakedMapIsValid = false;
...
cocos2d::Sprite* DynamicLight3::getFinalShadowMapSprite() {
	return finalShadowMapSprite;
}

int DynamicLight3::getLightSize() {
	return lightSize;
}

cocos2d::Vec2 DynamicLight3::getLightPosition() {
	return getPosition();
}

cocos2d::Sprite* DynamicLight3::getLightCircle() {
	return lightCircle;
}

void DynamicLight3::setLightCircle(cocos2d::Sprite* circle) {
	CC_SAFE_RELEASE(lightCircle);

	lightCircle = Sprite::createWithTexture(circle->getTexture());
	lightCircle->setAnchorPoint(circle->getAnchorPoint());
	lightCircle->setPosition(getPosition());
	lightCircle->setContentSize({ (float)lightSize, (float)lightSize });
	lightCircle->retain();
}